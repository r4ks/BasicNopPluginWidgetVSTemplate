...
namespace $ucprojectname$.Services.Common
{
    ...
    public partial class PluginPdfService : IPluginPdfService
    {
        ...
        /// <param name="$lcent$s">Products</param>
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task Print$Entity$sToPdfAsync(Stream stream, IList<$Entity$> $lcent$s)
        {
            if (stream == null)
            ...
            var productNumber = 1;
            var prodCount = $lcent$s.Count;

            foreach (var $lcent$ in $lcent$s)
            {
                var productName = await _localizationService.GetLocalizedAsync($lcent$, x => x.Name, lang.Id);
                var productDescription = await _localizationService.GetLocalizedAsync($lcent$, x => x.Description, lang.Id);
                ...
                var pictures = await _pictureService.GetPicturesByProductIdAsync($lcent$.Id);
                if (pictures.Any())
                {
